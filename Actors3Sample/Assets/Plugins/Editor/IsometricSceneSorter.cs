...
public class IsometricSceneSorter : EditorWindow {
    ...
    private float precissionSorting = -100;

    [SerializeField]
    //private bool sortOnPlay = false;

    [MenuItem ("Window/DEPTH SORTING", false, 10000)]
    static void ShowWindow () {
        EditorWindow.GetWindow<IsometricSceneSorter> ();
    }

    void OnEnable () {
        this.titleContent.text = "DEPTH SORTING";
    ...
    void OnGUI () {
        using (new EditorGUILayout.VerticalScope ()) {
            ...
            if (SortingLayer.layers.Length > 0) {
                ...
                precissionSorting = EditorGUILayout.FloatField(precissionSorting);
            }

            //This Button runs the main method to sort the sprites given their Y value
            if (GUILayout.Button ("Sort Sprite Units")) {
                SortSpritesRenderers ();
            }
    ...
    private void SortSpritesRenderers () {
        ...
        if (layerIndex >= SortingLayer.layers.Length) {
            ...
            for (int i = 0; i < spriteRenderers.Length; i++) {
                if (spriteRenderers[i].sortingLayerID == selectedLayer.id && spriteRenderers[i].gameObject.tag == "DEPTH_SORTING") {
                    spriteRenderers[i].sortingOrder = (int) (spriteRenderers[i].gameObject.transform.position.y * precissionSorting);
                    sortedSpritesCount++;
                }
            }
